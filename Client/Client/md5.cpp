...
inline void FF(uint &a, uint b, uint c, uint d, uint m, uint s, uint t)
{
	a = b + left_move((a + F(b, c, d) + m + t), s);
}
inline void GG(uint &a, uint b, uint c, uint d, uint m, uint s, uint t)
{
	a = b + left_move((a + G(b, c, d) + m + t), s);
}
inline void HH(uint &a, uint b, uint c, uint d, uint m, uint s, uint t)
{
	a = b + left_move((a + H(b, c, d) + m + t), s);
}
inline void II(uint &a, uint b, uint c, uint d, uint m, uint s, uint t)
{
	a = b + left_move((a + I(b, c, d) + m + t), s);
}
//压缩函数
inline void process(uint M[])
{
...
//填充至 bit=448 mod512
inline uint* padding(string str)
{
...
inline char* MD5Encryption(char* ptext)
{
	A_temp = A;
	B_temp = B;
	C_temp = C;
