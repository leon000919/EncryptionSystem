...
//进制转换(16进制转2进制)
inline void base_conversion(char* intext, char* outtext)
{
...
//E-BOX
inline void e_box(char* rp,char* temp)
{
	//首尾特殊
	temp[0] = rp[31];
	temp[47] = rp[0];
...
inline void extend_key(char* intext,char* outtext)
{
...
//PC-1
inline void pc_1(char* key,char* temp)
{
	for (int i = 0; i < 8; i++)
	{
		temp[i] = key[56 - 8 * i];
...
inline void pc_2(char* l_key, char* r_key,char* key)
{
	char temp[70] = {};
	for (int i = 0; i < 28; i++)
	{
...
//密钥与扩展后的RP异或
inline void k_xor_p(char* key,char* text)
{;
	for (int i = 0; i < 48; i++)
	{
...
// S-BOX 
inline void s_box(char* text,char* rtext)
{
	//8个4行16列的盒子
	int box[8][4][16] = {
		{ 
...
inline char* FT(char* lp, char* rp)
{
	...
	for (int i = 0; i < 16; i++)
	{
		...
		if (sum < 10)
		{
			...
			result[i] = char(sum + 'A' - 10);
	}
	return result;
}
//加密(建立在明文、密钥均处理完成的基础上)
inline char* DESEncryption(char* text,char* k)
{
	char ptext[70] = {}, ktext[70] = {};
	base_conversion(text, ptext);
	extend_key(k, ktext);
...
//解密
inline char* DESDecryption(char* text, char* k)
{
	char ctext[70] = {}, ktext[70] = {};
	base_conversion(text, ctext);
	extend_key(k, ktext);
