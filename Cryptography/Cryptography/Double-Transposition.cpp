//Double-Transposition密码

#include<iostream>

using namespace std;

inline char* DoubleTranspositionEncryptions(char* text, char* ktext)
{
...
inline char* DoubleTranspositionDecryptions(char* text, char* ktext)
{
	char* result = (char*)malloc((strlen(text) + strlen(ktext) + 1) * sizeof(char));
	memset(result, 0, strlen(text) + strlen(ktext) + 1);
...
inline char* DoubleTranspositionEncryption(char* text, char* ktext)
{
	char* atext;
	char* btext;
	atext = strtok_s(ktext, ",", &btext);
	return DoubleTranspositionEncryptions(DoubleTranspositionEncryptions(text, atext), btext);
}

inline char* DoubleTranspositionDecryption(char* text, char* ktext)
{
	char* atext;
	char* btext;
	atext = strtok_s(ktext, ",", &btext);
	return DoubleTranspositionDecryptions(DoubleTranspositionDecryptions(text, btext), atext);
}